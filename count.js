...
// Unique Words
let uniques = new Map();
let uniqueWordCount = 1;
// All words in the text
let words = sonnet.match(/[\w']+/g);
// Iterate through `words` and build up an associative Map of unique words.
  for (let i = 0; i < words.length; i++) {
  let word = words[i];
  if (uniques.get(word)) {
uniques.set(word, uniqueWordCount+=1);
  } else {
uniques.set(word, uniqueWordCount);
  }
}
console.log(uniques)
